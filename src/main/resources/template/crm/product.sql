#namespace("crm.product")
    ...
    #sql("queryById")
       ...
       from wms_crm_product as CrmProductController
       LEFT JOIN wms_crm_product_category as sc on sc.category_id = CrmProductController.category_id
    ...
    #sql("queryProductPageList")
      ...
      FROM wms_crm_contract_product sccp
      LEFT JOIN productview as scp on scp.product_id = sccp.product_id
      where sccp.contract_id = ?
    #end
     #sql("queryByCategoryId")
      select count(*) from wms_crm_product where category_id = ?
    #end
    #sql ("queryCategoryByParentId")
      select count(*) from wms_crm_product_category where pid = ?
    #end
    #sql("getByNum")
    ...
    #sql("querySubtotalByContractId")
      ...
      round(((select SUM(subtotal) from wms_crm_contract_product WHERE contract_id = ccc.contract_id)/100*(100 - ccc.discount_rate) ),2)as money
      from wms_crm_contract as ccc
      where ccc.contract_id = ?
    #end
    #sql("querySubtotalByBusinessId")
      select distinct ccc.discount_rate,
      round((( select SUM(subtotal) from wms_crm_business_product WHERE business_id = ccc.business_id ) /100*(100 - ccc.discount_rate) ),2)as money
      from wms_crm_business as ccc
