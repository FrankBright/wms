#namespace("crm.customer")
    ...
    #sql("deleteByIds")
    delete from wms_crm_customer where customer_id = ?
    #end
    ...
    from wms_crm_business as a inner join wms_crm_customer as b inner join wms_crm_business_type as c inner join
    wms_crm_business_status as d
    ...
    select count(*) from wms_crm_business where customer_id = ?
    #end
    ...
    select count(*) from wms_crm_contacts where customer_id = ?
    #end
    ...
    from wms_crm_receivables_plan as a inner join wms_crm_customer as b
    inner join wms_crm_contract as c
    ...
    from wms_crm_receivables as a inner join wms_crm_contract as b inner join wms_admin_user as c
    ...
    ifnull((select sum(c.money) from `wms_crm_receivables` c where c.contract_id = a.contract_id and c.check_status = 2),0) as receivablesMoneyCount
    from wms_crm_contract as a inner join wms_crm_customer as b on a.customer_id = b.customer_id
    ...
    from wms_admin_user as a inner join wms_admin_dept as b on a.dept_id = b.dept_id
    where a.user_id = ?
    #end

    #sql ("lock")
    update wms_crm_customer
    ...
    UPDATE wms_crm_customer SET owner_user_id = null
    WHERE customer_id in (
        #for(i:ids)
             #(for.index > 0 ? "," : "")#para(i)
        #end
    ...
    select customer_id from wms_crm_customer as ccc
    WHERE owner_user_id != 0
        and deal_status ='未成交'
        and is_lock = 0
        and ((unix_timestamp(now()) - unix_timestamp(IFNULL((
		SELECT car.create_time
		 FROM wms_admin_record  as car
		where
    ...
    #end
    #sql ("deleteMember")
    update wms_crm_customer set rw_user_id = replace(rw_user_id,?,','),ro_user_id = replace(ro_user_id,?,',') where customer_id = ?
    #end
    #sql ("updateDealStatusById")
      update wms_crm_customer set deal_status = ? where customer_id = ?
    #end

    #sql ("queryBatchIdByIds")
    select batch_id from wms_crm_customer where customer_id in (
        #for(i:ids)
          #(for.index > 0 ? "," : "")#para(i)
        #end
    )
    ...
    update wms_crm_customer set owner_user_id = #para(userId),followup = 0,create_time = #para(createTime) where customer_id in (
        #for(i:ids)
          #(for.index > 0 ? "," : "")#para(i)
        #end
    )
