#namespace("work")
  ...
  #sql("queryTaskByWorkId")
    select a.*,ifnull(b.name,'未分组') as className,ifnull(b.class_id,0) as classId,b.order_num as classOrder,
      ...
      from wms_task a left join wms_work_task_class b on a.class_id = b.class_id where a.work_id = #para(workId)
  ...
  #sql("workStatistics")
    select count(*) as allCount,
      ...
      from wms_task where 1 = 1 and ishidden = 0 and work_id >0
      #if(workId)
  ...
  #sql("queryRoleList")
  select role_id,role_name,remark from wms_admin_role where role_type in (5,6) and status = 1
  #end

  #sql ("queryOwnerRoleList")
  select a.user_id,b.realname,a.role_id,c.role_name,b.img
  from wms_work_user as a left join wms_admin_user as b on a.user_id = b.user_id
  left join wms_admin_role as c on a.role_id = c.role_id
  where a.work_id = ?
  #end
  #sql ("queryRealmByRoleId")
    select concat((select realm from `wms_admin_menu` where menu_id = b.parent_id),':',b.realm) from `wms_admin_role_menu` a left join `wms_admin_menu` b on a.menu_id = b.menu_id where a.role_id = ? and b.menu_type = 3
  #end
  #sql("archList")
  select a.*,b.name as workName,(select count(*) from wms_task_comment where type_id = a.task_id and type = 1) as commentCount,
         ...
         (select count(*) from wms_task where pid = a.task_id) as childAllCount,
         (select count(*) from wms_admin_file where batch_id = a.batch_id) as fileCount
  from wms_task a left join wms_work b on a.work_id = b.work_id where a.work_id = ? and a.is_archive = 1 and a.ishidden = 0
  #end
  #sql("queryTaskFileByWorkId")
    ...
    where a.batch_id in (select batch_id from `wms_task` where work_id = #para(workId))
  #end
  #sql("leave")
    update `wms_task` set main_user_id = null where work_id =  #para(workId) and main_user_id = #para(userId);
  #end
  #sql("leave1")
    update `wms_task` set owner_user_id = replace(owner_user_id,concat(',',#para(userId),','),',') where work_id = #para(workId) and  owner_user_id like concat(',',#para(userId),',');
  #end
  #sql("getTaskOwnerOnWork")
    select user_id,img,realname from `wms_admin_user` where user_id in (select main_user_id from `wms_task` where ishidden = 0 and work_id in (?))
  #end
#end
