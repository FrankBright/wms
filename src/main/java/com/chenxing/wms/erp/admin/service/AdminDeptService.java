...
public class AdminDeptService {
    ...
    public List<Record> queryDeptTree(String type,Integer id) {
        ...
        List<Record> adminDeptList = Db.find("select dept_id as id,name,pid from wms_admin_dept");
    ...
    private List<Record> queryTopDeptList(Integer deptId) {
        List<Record> recordList = Db.find("select dept_id as id,name,pid from wms_admin_dept");
    ...
    public List<Record> queryDeptByAuth(Long userId) {
        ...
        //拥有最高数据权限
        if(list.contains(5)){
            return Db.find("select dept_id as id,name,pid from wms_admin_dept");
        }else {
            adminDepts.add(Db.findFirst("select dept_id as id,name,pid from wms_admin_dept where dept_id=?", BaseUtil.getUser().getDeptId()));
            if(list.contains(4)){
    ...
    public List<Record> queryDeptByParentDept(Integer deptId,Integer deepness){
        ...
        if(deepness>0){
            List<Record> records=Db.find("select dept_id as id,name,pid from wms_admin_dept where pid=?",deptId);
    ...
    private List<Record> queryDeptByParentUser(Long userId,Integer deepness){
        List<Record> recordList=new ArrayList<>();
        if(deepness>0){
            List<Record> records=Db.find("SELECT a.dept_id AS id,a.name,a.pid,b.user_id FROM wms_admin_dept as a LEFT JOIN wms_admin_user as b on a.dept_id=b.dept_id WHERE b.parent_id = ?",userId);
            recordList.addAll(records);
            records.forEach(record -> {
    ...
    public R deleteDept(String id) {
        Integer userCount = Db.queryInt("select count(*) from wms_admin_user where dept_id = ?", id);
        if(userCount>0){
            return R.error("该部门下有员工，不能删除！");
        }
        Integer childDeptCount = Db.queryInt("select count(*) from wms_admin_dept where pid = ?", id);
        if(childDeptCount>0){
            return R.error("该部门下有下级部门，不能删除！");
        }
        int delete = Db.delete("delete from wms_admin_dept where dept_id = ?", id);
