package com.chenxing.wms.common.config.redis;
...
public class JedisClusterImpl implements Redis {

    private JedisCluster jedisCluster;
    private ISerializer serializer;
    private IKeyNamingPolicy keyNamingPolicy;

    JedisClusterImpl(JedisCluster jedisCluster, ISerializer serializer, IKeyNamingPolicy keyNamingPolicy) {
    ...
    public String hmset(Object key, Map<Object, Object> hash) {
        ...
        try {
            ...
            hash.forEach((k, v) -> {
                para.put(fieldToBytes(k), valueToBytes(v));
            });
    ...
    public <T> T hget(Object key, Object field) {
        JedisCluster jedisCluster = getJedisCluster();
        try {
            return (T) valueFromBytes(jedisCluster.hget(keyToBytes(key), fieldToBytes(field)));
        } finally {
            close(jedisCluster);
        }
    }
    ...
    public List hmget(Object key, Object... fields) {
        JedisCluster jedisCluster = getJedisCluster();
        try {
            List<byte[]> data = jedisCluster.hmget(keyToBytes(key), fieldsToBytesArray(fields));
            return valueListFromBytesList(data);
        } finally {
            close(jedisCluster);
        }
    ...
    public Long hdel(Object key, Object... fields) {
        JedisCluster jedisCluster = getJedisCluster();
        try {
            return jedisCluster.hdel(keyToBytes(key), fieldsToBytesArray(fields));
        } finally {
            close(jedisCluster);
        }
    }
    ...
    public boolean hexists(Object key, Object field) {
        JedisCluster jedisCluster = getJedisCluster();
        try {
            return jedisCluster.hexists(keyToBytes(key), fieldToBytes(field));
        } finally {
            close(jedisCluster);
        }
    }
    ...
    public Map hgetAll(Object key) {
        ...
        try {
            ...
            data.forEach((k, v) -> {
                result.put(fieldFromBytes(k), valueFromBytes(v));
            });
            return result;
        } finally {
            close(jedisCluster);
    ...
    public Long hincrBy(Object key, Object field, long value) {
        JedisCluster jedisCluster = getJedisCluster();
        try {
            return jedisCluster.hincrBy(keyToBytes(key), fieldToBytes(field), value);
        } finally {
            close(jedisCluster);
        }
    }
    ...
    public Double hincrByFloat(Object key, Object field, double value) {
        JedisCluster jedisCluster = getJedisCluster();
        try {
            return jedisCluster.hincrByFloat(keyToBytes(key), fieldToBytes(field), value);
        } finally {
            close(jedisCluster);
        }
    }
    ...
    public Long getCounter(Object key) {
        JedisCluster jedisCluster = getJedisCluster();
        try {
            String ret = jedisCluster.get(keyNamingPolicy.getKeyName(key));
    ...
    public Long zadd(Object key, Map<Object, Double> scoreMembers) {
        ...
        try {
            ...
            scoreMembers.forEach((k, v) -> para.put(valueToBytes(k), v));
    ...
    @Override
    public byte[][] fieldsToBytesArray(Object... fieldsArray) {
